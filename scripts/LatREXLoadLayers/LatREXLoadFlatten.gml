...
function LatLoadREXPaintFlatten(_path, _symbolSprite, _transparent)
{
    static _system = __LatSystem();
    static _surface = -1;
    
    _path = gzip_unzip(_path, game_save_id);
    ...
    else if (_layerCount == 1)
    {
        ...
        repeat(_layerHeight)
        {
            ...
            repeat(_layerWidth)
            {
                ...
                if ((_unicode == 0) && (_bgRed == 255) && (_bgGreen == 0) && (_bgBlue == 255))
                {
        ...
        repeat(_layerCount)
        {
            ...
            repeat(_layerHeight)
            {
                ...
                repeat(_layerWidth)
                {
                    ...
                    {
                        draw_sprite_ext(__LatPixel, 0, _x, _y, LATTICE_CELL_WIDTH, LATTICE_CELL_HEIGHT, 0, (_bgBlue << 16) | (_bgGreen << 8) | _bgRed, 1);
                        draw_sprite_ext(_symbolSprite, _unicode, _x, _y, LATTICE_CELL_WIDTH, LATTICE_CELL_HEIGHT, 0, (_fgBlue << 16) | (_fgGreen << 8) | _fgRed, 1);
                    }
