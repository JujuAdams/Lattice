...
/// the ray will be considered (which is better UX when navigating using a directional). The
...
function GuiNavGetRaycast(_x, _y, _dX, _dY, _excludeArray = [])
{
    ...
    repeat(array_length(_stepOrder))
    {
        with(method_get_self(_stepOrder[_i]))
        {
            ...
            var _nearestX = clamp(_x, x, x + width-1);
            var _nearestY = clamp(_y, y, y + height-1);
